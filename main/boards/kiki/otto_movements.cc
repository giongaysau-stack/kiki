...
void Otto::ServoInit(int lf_angle, int rf_angle, int lb_angle, int rb_angle, int delay_time) {
    // Save home position for future reference
    servo_home_[0] = lf_angle;  // LF
    servo_home_[1] = rf_angle;  // RF
    servo_home_[2] = lb_angle;  // LB
    servo_home_[3] = rb_angle;  // RB
    ...
    ESP_LOGI(TAG, "Dog servo initialized - LF:%d RF:%d LB:%d RB:%d (home saved)", 
...
void Otto::StandUp() {
    ESP_LOGI(TAG, "Dog standing up to rest position (using calibrated home)");
    // Use saved home position instead of hard-coded 90°
    ServoInit(servo_home_[0], servo_home_[1], servo_home_[2], servo_home_[3], 2400);
...
void Otto::DogLieDown(int delay_time) {
    ...
    // Gradually lower all legs to lie flat - very slow and gentle (half speed)
    // Use longer delay for smoother transition
    int smooth_delay = (delay_time < 1000) ? 3000 : delay_time * 2;
