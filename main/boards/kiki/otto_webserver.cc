...
void send_otto_control_page(httpd_req_t *req) {
    ...
    httpd_resp_sendstr_chunk(req, "<div class='nav-tab' onclick='window.location.href=\"/servo_calibration\"' id='tab3'>⚙️ Servo</div>");
    httpd_resp_sendstr_chunk(req, "</div>");
    ...
    // AI Chat Section - From kytuoi repository
    httpd_resp_sendstr_chunk(req, "<div class='movement-section'>");
    ...
    httpd_resp_sendstr_chunk(req, "</div>");
    httpd_resp_sendstr_chunk(req, "<textarea id='ai_text_input' placeholder='Nhập văn bản để gửi lên server...' rows='3' style='width:100%;'></textarea>");
    httpd_resp_sendstr_chunk(req, "<div style='margin-top:10px;'>");
    httpd_resp_sendstr_chunk(req, "  <button class='btn' onclick='sendTextToAI()'>Send Text to AI</button>");
    httpd_resp_sendstr_chunk(req, "</div>");
    httpd_resp_sendstr_chunk(req, "<div id='ai-text-status' style='margin-top:10px; padding:5px; background:#f0f0f0; border-radius:4px; display:none;'></div>");
    httpd_resp_sendstr_chunk(req, "</div>");
    httpd_resp_sendstr_chunk(req, "</div>");
    ...
    */
    ...
    // AI text chat function - Original from kytuoi repository
    httpd_resp_sendstr_chunk(req, "async function sendTextToAI() {");
    httpd_resp_sendstr_chunk(req, "  const text = document.getElementById('ai_text_input').value.trim();");
    httpd_resp_sendstr_chunk(req, "  if (!text) {");
    httpd_resp_sendstr_chunk(req, "    showStatus('Please enter text', true);");
    httpd_resp_sendstr_chunk(req, "    return;");
    httpd_resp_sendstr_chunk(req, "  }");
    httpd_resp_sendstr_chunk(req, "  const statusDiv = document.getElementById('ai-text-status');");
    httpd_resp_sendstr_chunk(req, "  statusDiv.style.display = 'block';");
    httpd_resp_sendstr_chunk(req, "  statusDiv.textContent = 'Sending text to AI...';");
    ...
    httpd_resp_sendstr_chunk(req, "    if (data.success) {");
    httpd_resp_sendstr_chunk(req, "      statusDiv.textContent = 'Text sent to AI! Check device response.';");
    httpd_resp_sendstr_chunk(req, "      statusDiv.style.background = '#d4edda';");
    httpd_resp_sendstr_chunk(req, "      showStatus('Text sent successfully!', false);");
    httpd_resp_sendstr_chunk(req, "    } else {");
    httpd_resp_sendstr_chunk(req, "      statusDiv.textContent = 'Failed: ' + data.message;");
    httpd_resp_sendstr_chunk(req, "      statusDiv.style.background = '#f8d7da';");
    httpd_resp_sendstr_chunk(req, "      showStatus('Failed: ' + data.message, true);");
    httpd_resp_sendstr_chunk(req, "    }");
    httpd_resp_sendstr_chunk(req, "  } catch (e) {");
    httpd_resp_sendstr_chunk(req, "    statusDiv.textContent = 'Error: ' + e.message;");
    httpd_resp_sendstr_chunk(req, "    statusDiv.style.background = '#f8d7da';");
    httpd_resp_sendstr_chunk(req, "    showStatus('Error: ' + e.message, true);");
    httpd_resp_sendstr_chunk(req, "  }");
    httpd_resp_sendstr_chunk(req, "  setTimeout(() => {");
    httpd_resp_sendstr_chunk(req, "    statusDiv.style.display = 'none';");
    httpd_resp_sendstr_chunk(req, "  }, 5000);");
    httpd_resp_sendstr_chunk(req, "}");
...
esp_err_t otto_send_text_to_ai_handler(httpd_req_t *req) {
    ...
    if (content_len == 0 || content_len > 2048) {
        ESP_LOGE(TAG, "Invalid content length: %d", (int)content_len);
...
esp_err_t otto_servo_calibration_handler(httpd_req_t *req) {
    ...
    httpd_resp_sendstr_chunk(req, "<button onclick='saveCalibration()'>💾 Save as Home Position</button>");
    ...
    httpd_resp_sendstr_chunk(req, "const lf=document.getElementById('lf').value;");
    httpd_resp_sendstr_chunk(req, "const rf=document.getElementById('rf').value;");
    httpd_resp_sendstr_chunk(req, "const lb=document.getElementById('lb').value;");
    httpd_resp_sendstr_chunk(req, "const rb=document.getElementById('rb').value;");
    httpd_resp_sendstr_chunk(req, "fetch(`/servo_save?lf=${lf}&rf=${rf}&lb=${lb}&rb=${rb}`).then(r=>r.json()).then(d=>{");
    httpd_resp_sendstr_chunk(req, "let s=document.getElementById('status');");
    ...
    httpd_resp_sendstr_chunk(req, "setTimeout(()=>s.style.display='none',3000);");
    httpd_resp_sendstr_chunk(req, "});");
    httpd_resp_sendstr_chunk(req, "}");
    httpd_resp_sendstr_chunk(req, "function resetToDefault(){");
    httpd_resp_sendstr_chunk(req, "['lf','rf','lb','rb'].forEach(s=>{");
    httpd_resp_sendstr_chunk(req, "document.getElementById(s).value=90;updateServo(s,90);");
    httpd_resp_sendstr_chunk(req, "});");
    httpd_resp_sendstr_chunk(req, "}");
    httpd_resp_sendstr_chunk(req, "window.onload=function(){");
    httpd_resp_sendstr_chunk(req, "fetch('/servo_get').then(r=>r.json()).then(d=>{");
...
esp_err_t otto_servo_get_handler(httpd_req_t *req) {
    ...
    char response[200];
    snprintf(response, sizeof(response), 
        "{\"lf\":%d,\"rf\":%d,\"lb\":%d,\"rb\":%d}", (int)lf, (int)rf, (int)lb, (int)rb);
    httpd_resp_sendstr(req, response);
    return ESP_OK;
}
...
esp_err_t otto_servo_save_handler(httpd_req_t *req) {
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGI(TAG, "💾 Saved servo calibration (RAW): LF=%d RF=%d LB=%d RB=%d", lf, rf, lb, rb);
...
esp_err_t otto_start_webserver(void) {
    ...
    config.max_uri_handlers = 25;  // Added servo calibration handlers (4 new)
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        */
        ...
        ESP_LOGI(TAG, "HTTP server started successfully (with UDP Drawing + Gemini API + Servo Calibration support)");
